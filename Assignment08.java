...
class Assignment08 {
	...
	public static void main(String[] args) throws IOException {
		...
		while (menuChoice != 8) {
			...
			(menuChoice == 2)        listAllPlayers(roster);
			else if (menuChoice == 3) {
				listTopAttackers(roster);
			}
			else if (menuChoice == 4) listTopBlockers(roster);
			else if (menuChoice == 5) addPlayer(roster);
			
			else if (menuChoice == 6)        changePlayerStats(roster);
			else if (menuChoice == 7)        countPlayers(roster);
	...
	static String getPlayerName() {
		...
		String lastName = scnr.next();    return firstName + " " + lastName;  
		}  
